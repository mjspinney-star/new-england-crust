---
title: "Backyard Neapolitan Margherita (60-Second Bake)"
description: "The classic that every outdoor pizza oven was built for — 00 flour, San Marzano tomatoes, fresh mozzarella, and a bake measured in seconds."
pubDate: 2026-09-07
heroImage: "/images/recipes/margherita-hero.jpg"
style: "neapolitan"
prepTime: "PT30M"
cookTime: "PT2M"
restTime: "PT24H"
totalTime: "PT24H32M"
servings: "3 twelve-inch pies"
ingredients:
  - "500 g 00 flour"
  - "310 g cool water (62% hydration)"
  - "12 g fine sea salt"
  - "1 g instant yeast (yes, that little — time does the work)"
  - "1 can (28 oz) whole San Marzano tomatoes, hand-crushed with 1 tsp salt (never cooked)"
  - "250 g fresh mozzarella, torn and drained on paper towels"
  - "Fresh basil leaves"
  - "Olive oil for finishing"
instructions:
  - step: "Mix and slow ferment"
    text: "Mix, knead 8 minutes until silky, and ferment: 4 hours at room temp, then ball into three 270 g balls and rest 18–24 hours in the fridge (or all day on the counter, the traditional way)."
  - step: "Preheat like you mean it"
    text: "Koda 12: fire to 850–900°F with the stone fully saturated — a solid 20–25 minutes, not 10. Ninja Woodfire: max pizza setting, full preheat. An infrared thermometer takes the guessing out either way."
  - step: "Stretch gently"
    text: "Press from the center out, preserving the puffy rim. Neapolitan dough is delicate — knuckle-stretch, don't roll."
  - step: "Top light, launch fast"
    text: "Thin layer of crushed tomatoes, scattered mozzarella, a few basil leaves, thread of oil. Launch immediately — a loaded pie glues itself to the peel in under a minute."
  - step: "Bake — two ovens, two clocks"
    text: "Koda 12: 60–90 seconds, turning every 20. Ninja Woodfire: tops out around 700°F, so plan on 3–4 minutes — you trade some leopard-spotting for a slightly crisper base, and it is still a legitimately great pie. Pull when the cornicione is puffed and spotted."
gear:
  - name: "Ooni Koda 12"
    url: "https://YOUR-AFFILIATE-LINK"
    note: "Hits 950°F and its 12-inch stone is exactly the size a classic Neapolitan wants to be. The oven this recipe was tested on."
  - name: "Ninja Woodfire Outdoor Oven"
    url: "https://YOUR-AFFILIATE-LINK"
    note: "The convenience play — tops out around 700°F, so use the 3–4 minute timing in the bake step."
  - name: "Infrared thermometer"
    url: "https://YOUR-AFFILIATE-LINK"
    note: "The difference between a 90-second pie and a pale one is stone temp you can't see. This tool ends the guessing."
  - name: "12-inch perforated pizza peel"
    url: "https://YOUR-AFFILIATE-LINK"
    note: "Sized to the Koda 12, and the perforations shed excess flour so it doesn't burn bitter on the stone."
keywords:
  - "Neapolitan pizza recipe"
  - "Ooni margherita recipe"
  - "pizza oven recipe"
  - "00 flour pizza dough"
draft: true
---

This is the pizza the backyard oven was invented for, and the one every new oven owner should master first — because it teaches heat management, launching, and turning, the skills every other outdoor recipe on this site builds on.

It's also the best oven-comparison recipe there is, which is exactly what we'll do with it: same dough, same toppings, three different ovens. Results coming in a follow-up post.
