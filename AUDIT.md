...
2. **`chicken-bacon-ranch` gear card never renders its link.** Its `gear:` entry has `url: https://amzn.to/4obF5K3` + note "live Amazon Associates link" but **no `affiliateId`**, and `RecipeLayout` only renders a link when `affiliateId` is present — so the page shows "Pending affiliate link" despite a live URL existing. Same pattern risk in other recipes' gear entries. **[D]**
...
5. `RecipeLayout.astro:285` — the email-capture form has `action="TODO"`; submitting it errors. Not affiliate, but it's a live broken form on every structured recipe. **[F]**
